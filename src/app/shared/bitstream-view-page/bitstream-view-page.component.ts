...
@Component({
  selector: 'ds-bitstream-view-page',
  templateUrl: './bitstream-view-page.component.html'
})
/**
 * Page component for downloading a bitstream
 */
export class BitstreamViewPageComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.bitstream$.pipe(
      switchMap((bitstream: Bitstream) => {
        const isAuthorized$ = this.authorizationService.isAuthorized(FeatureID.CanView, isNotEmpty(bitstream) ? bitstream.self : undefined);
      ...
      switchMap(([isAuthorized, isLoggedIn, bitstream]: [boolean, boolean, Bitstream]) => {
        if (isAuthorized && isLoggedIn) {
          return this.fileService.retrieveFileDownloadLink(bitstream._links.view.href).pipe(
      ...
      if (isAuthorized && isLoggedIn && isNotEmpty(fileLink)) {
        ...
        this.hardRedirectService.redirect(bitstream._links.view.href);
      } else if (!isAuthorized && isLoggedIn) {
